function viewlick_opto(root, cbID,varargin)
%VIEWLICK_OPTO   Lick raster and PSTH.
%   VIEWLICK_OPTO(CBID,'TRIGGERNAME',TRIGEVENT,'SORTEVENT',SEVENT,'EVENTTYPE
...
switch g.eventtype
    case 'stim'
        datapath = fullfile(root,animalID,sessionID,'StimEvents.mat');
        TE = load(datapath);
    case {'event','behav'}
        datapath = fullfile(root,animalID,sessionID,'TE.mat');
        TE = load(datapath);
end
...
% Save
saveas(gcf,fullfile('E:\HDB_Ach\Fiber_photometry','Plots', [animalID '_' sessionID '.jpg']))
close(gcf)